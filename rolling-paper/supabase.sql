...
-- 이미 messages 테이블을 만든 적이 있다면 이 파일을 다시 실행해도 안전합니다
...
alter table messages add column if not exists secret text;
...
drop trigger if exists enforce_message_language on messages;
